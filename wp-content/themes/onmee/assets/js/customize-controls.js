/* global onmeeBgColors, onmeeColor, jQuery, wp, _ */
/**
...
( function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( onmeeBgColors ).forEach( function( context ) {
					...
					if ( onmeeBgColors[ context ].color ) {
						backgroundColorValue = onmeeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( onmeeBgColors[ context ].setting ).get();
					}
					onmeeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( onmeeBgColors ).forEach( function( context ) {
			wp.customize( onmeeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					...
					onmeeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		onmeeSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				onmeeSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function onmeeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = onmeeColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function onmeeSetRetineLogoVisibility( visible ) {
